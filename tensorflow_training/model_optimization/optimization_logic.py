...
def optimize_model(model, loss, optimizer, loss_weights, metrics, callbacks, config):
    ...
    if qtn_layer and (not is_model_alr_qnt):
        model = tf.keras.models.clone_model(
            model,
            clone_function=partial(
                apply_quantization_to_layer,
                quantize_layer_names=config["optimization"]["prune"][
    ...
    clustering_params = {
        ...
        "cluster_centroids_init": rgetattr(
            tfmot, config["optimization"]["cluster"]["CentroidInitialization"]
        ),
    }
    use_clustering = config["optimization"]["cluster"]["use_clustering"]
    is_model_alr_clustered = config["optimization"]["cluster"][
        "is_model_already_clustered"
    ]
    cluster_layers = config["optimization"]["cluster"]["cluster_layers"]

    if use_clustering and (not is_model_alr_clustered):
        model = tfmot.clustering.keras.cluster_weights(model, **clustering_params)
    ...
    if cluster_layers and (not is_model_alr_clustered):
        model = tf.keras.models.clone_model(
            ...
            clone_function=partial(
                ...
                cluster_layer_names=config["optimization"]["cluster"][
                    "cluster_layer_names"
                ],
            ),
        )
    ...
    end_step = np.ceil(total / bsize).astype(np.int32) * config["trainer"]["epochs"]
    pruning_params = {
    ...
    if prune_layers:
        ...
        pruning_callbacks = [
            tfmot.sparsity.keras.UpdatePruningStep(),
            tfmot.sparsity.keras.PruningSummaries(
                log_dir=config["trainer"]["tf_prune_logs"]
            ),
        ]
        callbacks.append(pruning_callbacks)
