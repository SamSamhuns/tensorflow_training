...
def create_custom_module(model, model_info):
    ...
    class CustomModule(tf.Module):
        ...
        @tf.function(
            input_signature=[tf.TensorSpec(shape=(None, w, h, c), dtype=tf.uint8)]
        )
...
def convert_h5_to_savedmodel(
    config_path: str, h5_model_path: str, savedmodel_export_dir: str
):
    ...
    try:
        tf.saved_model.save(
            custom_module,
            savedmodel_export_dir,
            signatures={"serving_default": custom_module.update_signature},
        )
        print(
            "\x1b[6;30;42m"
            + f"Export to {savedmodel_export_dir} successful."
            + "\x1b[0m"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        "--savedmodel_export_dir",
        type=str,
        default="checkpoints/1/model.savedmodel",
        help="savedmodel export dir path. (default %(default)s)",
    )
    args = parser.parse_args()
    convert_h5_to_savedmodel(
        args.config, args.h5_model_path, args.savedmodel_export_dir
    )
